...
gt.tbl.AE_crossover <- tbl.AE_crossover %>% as_gt()
gt::gtsave(gt.tbl.AE_crossover, file = "./image/tbl.AE_crossover.png")
```
...
# Convert tbl_regression to gt for more flexible handling
gt.tbl.Adhere_crossover <- tbl.Adhere_crossover %>% as_gt()
gt::gtsave(gt.tbl.Adhere_crossover, file = "./image/tbl.Adhere_crossover.png")
```
No significant crossover effect found.
...
# Convert tbl_regression to gt for more flexible handling
gt.tbl.PK.crossover.model.bviral <- tbl.PK.crossover.model.bviral %>% as_gt()
gt::gtsave(gt.tbl.PK.crossover.model.bviral, file = "./image/tbl.PK.crossover.model.bviral.png")
...
# Convert tbl_regression to gt for more flexible handling
gt.tbl.PK.crossover.model.sviral <- tbl.PK.crossover.model.sviral %>% as_gt()
gt::gtsave(gt.tbl.PK.crossover.model.sviral, file = "./image/tbl.PK.crossover.model.sviral.png")
...
broom::tidy(model.Adhere.product.1,exponentiate = TRUE,conf.int = TRUE) %>% kable() %>% kable_styling() %>%
  save_kable(file = "./image/ad_product_type_model.png")
```

```{r}
model.Adhere.freq.12 <- geepack::geeglm(cbind(Adhere, nonAdhere) ~ period + week+ age*freq + gender*freq + race*freq, data = endpoints.Adhere.regimen, family = binomial, id = ptid, corstr = "independence")
...
AIC(model.Adhere.freq.12)
# AIC(model.Adhere.freq.2)
# AIC(model.Adhere.freq.3)

#summary(model.Adhere.freq.1) 
broom::tidy(model.Adhere.freq.12,exponentiate = TRUE,conf.int = TRUE) %>% kable()%>% kable_styling() %>%
  save_kable(file = "./image/ad_regimen_model.png")
# summary(model.Adhere.freq.1)
# summary(model.Adhere.freq.1)
```
